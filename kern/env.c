...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	// LAB 3: Your code here.
	struct Elf *elfdir = (struct Elf *) binary;
	struct Proghdr *ph, *eph;
	if (elfdir->e_magic != ELF_MAGIC)
		panic("elf header magic is not correct\n");
	ph = (struct Proghdr *) ((uint8_t *)elfdir + elfdir->e_phoff);	
	eph = ph + elfdir->e_phnum;
	...
	for (; ph < eph; ph++) {
		...
		if (ph->p_filesz > ph->p_memsz)
			panic("filesz is more than memsz\n");
		region_alloc(e, (void *) ph->p_va, ph->p_memsz);
		memcpy((void *) ph->p_va, (void *) binary + ph->p_offset, ph->p_filesz);
		memset((void *) ph->p_va + ph->p_filesz, 0, ph->p_memsz - ph->p_filesz);
	}
	
	e->env_tf.tf_eip = elfdir->e_entry;
